...
export class DropdownFieldComponent extends InjectReactiveForm implements OnInit {
  @Input() label: string = '';
  @Input() options: IOption[] = [];
  @Input() formField: string = '';

  public defaultValue: IOption = {label: EDefaultValue.OptionLabel, value: ''};
  ...
  override ngOnInit(): void {
    super.ngOnInit();
    const cashedValue = this.options.find((option: IOption): boolean => option.value === this.form.value[this.formField]);
    if (cashedValue) {
